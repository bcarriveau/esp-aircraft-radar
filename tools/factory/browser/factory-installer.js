...
async function eraseAndInstall() {
  ...
  try {
    ...
    log("All factory images written successfully. Applying explicit ESP32-S3 EN reset pulse.");
    // Field testing on the Waveshare ESP32-S3-Touch-LCD-7 showed that the
    // generic esptool-js hard-reset completion could release Web Serial while
    // leaving the board stopped until its physical RESET button was pressed.
    // Drive IO0 inactive, hold EN low long enough to match a physical reset,
    // then release EN and give the S3 time to begin booting before closing the
    // serial port. esptool-js maps DTR=true to IO0 low and RTS=true to EN low.
    await state.transport.setDTR(false);
    await state.transport.setRTS(true);
    await new Promise((resolve) => setTimeout(resolve, 250));
    await state.transport.setRTS(false);
    await new Promise((resolve) => setTimeout(resolve, 750));
    log("Reset pulse complete; releasing reset/boot control lines and closing Web Serial.");
    await releaseSerialTransport({ updateStatus: false });
    setStatus(el.deviceStatus, "USB serial released.", "ok");
    setStatus(el.installStatus, `${state.manifest.version_label} FACTORY INSTALL COMPLETE\nReset pulse sent and USB serial released. The radar should boot into new-owner setup. If the display remains stopped, press RESET once and report it.`, "ok");
    log("Factory install complete; explicit reset pulse sent and Web Serial released.");
  } catch (error) {
