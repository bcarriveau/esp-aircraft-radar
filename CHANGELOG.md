...
- **Current commit:** `e5afe84d8bb0cd56eb19900437b9a347e7605ee7`
...
The checked-in `release/` directory contains many historical Product-numbered
`.radarota` packages from development and verification.

Those files are not all current releases. Old Product packages should be treated as
historical/stale unless deliberately used for rollback/testing. The fixed-name
`release/firmware.radarota` is generated local-install output and can also be stale
if it was not regenerated by the exact source being built. Current identity comes
from `include/build_info.h`, the intended commit/tag, and the generated matching
manifest/package—not from file presence alone.

Repository cleanup of historical release binaries is separate housekeeping and is
not performed by this documentation catch-up.
